...
% LAT_lim = [0 60];
% LON_lim = [100 190];
%% Find the days in summer
first_date = datetime(start_year,06,01);
last_date = datetime(end_year,08,31);
date_array = first_date:days(1):last_date;
summer_ind = find(month(date_array)>=6 & month(date_array)<=8);

date_summer = date_array(summer_ind);
index_num = length(date_summer);
%%
lon_input = [];
lat_input = [];
for i = 1:index_num
    %%
    the_date = date_summer(i);
...
saveas(fig,['./image/probabilities_distribution/Summer/' ...
    num2str(start_year) '-' num2str(end_year) '_Summer.png']);