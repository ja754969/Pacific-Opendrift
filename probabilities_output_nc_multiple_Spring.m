...
% LAT_lim = [0 60];
% LON_lim = [100 190];
%% Find the days in spring
first_date = datetime(start_year,03,01);
last_date = datetime(end_year,05,30);
date_array = first_date:days(1):last_date;
spring_ind = find(month(date_array)>=3 & month(date_array)<=5);
...
%         yyyy '_' MM '_' dd '.nc'];
    filename = ['D:/Data/used_by_projects/Pacific-Opendrift/' ...
        'nc_output/Kuroshio_upstream_path/Opendrift_Kuroshio_Luzon_path_' ...
        yyyy '_' MM '_' dd '.nc'];
...
%     lat_input_i = lat(stranded_trajectory,:); % stranded only (lat)
    ...
    %%
    lon_init_i = lon(:,1); % initial active and stranded (lon)
    lat_init_i = lat(:,1); % initial active and stranded (lat)
...
hold on;
for ind = 1:length(lon_init)
%     hold on;
    init = m_plot(lon_init(ind,1),lat_init(ind,1),'Marker','x','MarkerSize',15,'Color',[0.7 0.7 0.7]);
    hold on;
...
title([num2str(start_year) ' - ' num2str(end_year) ' Spring'],'FontSize',15)
% lgd = legend([init active],{['initial (' num2str(not_stranded_count) ')'],...
%     ['active (' num2str(not_stranded_count) ')']});
saveas(fig,['./image/probabilities_distribution/Spring/' ...
    num2str(start_year) '-' num2str(end_year) '_Spring_Kuroshio_upstream_path.png']);
% saveas(fig,['./image/probabilities_distribution/Spring/' ...
%     num2str(start_year) '-' num2str(end_year) '_Spring_Kuroshio_Luzon_path.png']);