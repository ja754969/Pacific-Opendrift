...
% LAT_lim = [0 60];
% LON_lim = [100 190];
%% Find the days in fall
first_date = datetime(start_year,06,01);
last_date = datetime(end_year,08,31);
date_array = first_date:days(1):last_date;
fall_ind = find(month(date_array)>=9 & month(date_array)<=11);

date_fall = date_array(fall_ind);
index_num = length(date_fall);
%%
lon_input = [];
lat_input = [];
for i = 1:index_num
    %%
    the_date = date_fall(i);
...
title([num2str(start_year) ' - ' num2str(end_year) ' Fall'],'FontSize',15)
% saveas(fig,['./image/probabilities_distribution/Fall/' ...
%     num2str(start_year) '-' num2str(end_year) '_Fall.png']);